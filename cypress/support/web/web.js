let amountBeforeTransferAcc1;
let amountBeforeTransferAcc2;

Cypress.Commands.add('setDefaultMass', (email, name, account, password) => {
  cy.window().then((win) => {
    win.localStorage.setItem(email, `{"name":"${name}","email":"${email}","password":"${password}","accountNumber":"${account}","balance":2000,"logged":false}`);
    win.localStorage.setItem(`transaction:${email}`, '[{"id":"05aa2121-f38e-4d20-9781-6ad30f0f3691","date":"19/07/2023","type":"Abertura de conta","transferValue":1000,"description":"Saldo adicionado ao abrir conta"}]');
  });
});
...
Cypress.Commands.add('validateRegister', () => {
...
Cypress.Commands.add('transferToAccount', (userToTransfer, userRegistered, valueToTranfer) => {
  let dataTranfer
  ...
  cy.getAllLocalStorage().then(res => {
    cy.wrap(res["https://bugbank.netlify.app"][userToTransfer]).then(data => {
      ...
      amountBeforeTransferAcc1 = dataTranfer.balance;
      ...
      cy.get('.styles__ContainerFormTransfer-sc-1oow0wh-0 > :nth-child(2) > .input__default').click({ force: true }).type(valueToTranfer);
    ...
    cy.wrap(res["https://bugbank.netlify.app"][userRegistered]).then(data => {
      dataTranfer = JSON.parse(data);
      amountBeforeTransferAcc2 = dataTranfer.balance;
    });
  });
});

Cypress.Commands.add('validateTransfer', () => {
  cy.get('#modalText').invoke("text").should("equal", "Transferencia realizada com sucesso");
});

Cypress.Commands.add('validateAmount', (userToTransfer, userRegistered, valueToTranfer) => {
  cy.getAllLocalStorage().then(res => {
    cy.wrap(res["https://bugbank.netlify.app"][userToTransfer]).then(data => {
      data = JSON.parse(data);
      expect(data.balance).to.equal(amountBeforeTransferAcc1 + valueToTranfer);
    });
    cy.wrap(res["https://bugbank.netlify.app"][userRegistered]).then(data => {
      data = JSON.parse(data);
      expect(data.balance).to.equal(amountBeforeTransferAcc2 - valueToTranfer);
    });
  });
});
...
Cypress.Commands.add('validateLogout', () => {
